...
export const handler = async (event: LogSheetEntryEvent) => {
  const args = event.arguments ?? {};
  const { date, team1p1, team1p2, team2p1, team2p2, winner } = args;

  if (!team1p1 || !team1p2 || !team2p1 || !team2p2 || !winner) {
    throw new Error('All player fields and winner are required');
  }
  ...
  if (!credentials.client_email || !credentials.private_key) {
    throw new Error(
      `GOOGLE_SERVICE_ACCOUNT_KEY is missing client_email or private_key. paramName=${paramName} format=${parsed.format} diagnostics=${JSON.stringify(secretDiagnostics)}`
    );
  }
